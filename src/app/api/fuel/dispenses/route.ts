...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Error fetching fuel dispenses:', error)
    return NextResponse.json(
      { error: 'Failed to fetch fuel dispenses' },
      { status: 500 }
    )
  }
}

// POST create new fuel dispense
export async function POST(request: NextRequest) {
  try {
    const body = await request.json()
    const { busId, date, quantity, odometerReading, dispensedBy, notes } = body
    ...
    // Check if there's enough fuel in stock
    ...
    const dispense = await prisma.fuelDispense.create({
      data: {
        busId,
        date: new Date(date),
    ...
    console.error('Error creating fuel dispense:', error)
    return NextResponse.json(
      { error: 'Failed to create fuel dispense', details: error.message },
      { status: 500 }
    )
  }
}

// DELETE fuel dispense
export async function DELETE(request: NextRequest) {
  try {
    ...
    await prisma.fuelDispense.delete({
      where: { id },
    })

    return NextResponse.json({ message: 'Dispense deleted successfully' })
  } catch (error: any) {
    console.error('Error deleting fuel dispense:', error)
